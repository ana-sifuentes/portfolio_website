...
const createMainChartTable = () => {
    const headersText = ['', 'Custo', '%', 'Procedimentos', 'Mortes'];
...
const updateMainChartTable = (group) => {
    const location = d3.select('#main-chart-location').property('value');
    ...
    groupValues = {
        name: group,
        cost: formatMainChartValue(groupData.map((d) => d.cost)[0], 'cost'),
    ...
    detailedData.forEach((d, i) => {
        const values = {
            name: d['Desfecho'],
            cost: formatMainChartValue(d['Custo Atr.'], 'cost'),
            percent: formatMainChartValue(
...
const updateMainChart = () => {
    ...
    mainChart.svg
        ...
        .on('click', (e) => updateMainChartTable(d3.select(e.target).text()));

...
const formatMapChartValue = (value, indicator) => {
    if (indicator === 'percent') return `${(value * 100).toFixed(2)}%`;
    ...
    if (value < 1e3) {
        ...
        return 'R$ ' + (value / 1e3).toFixed(2) + ' mil';
    } else if (value < 1e9) {
        return 'R$ ' + (value / 1e6).toFixed(2) + ' milhões';
    } else {
        return 'R$ ' + (value / 1e9).toFixed(2) + ' bilhões';
    }
};
...
const showMapChartTooltip = (e, d) => {
    ...
    if (mapChart.indicator === 'cost') {
        ...
        label2.text('Custo corrigido por habitante:');
        value2.text(data.cost.toFixed(2));
        ...
        value2.text(data.deaths.toFixed(1));
    }
};
...
const getMapChartLegendText = (value) => {
    if (mapChart.indicator === 'cost') return 'R$ ' + value.toFixed(2);

    if (mapChart.indicator === 'percent') return `${(value * 100).toFixed(1)}%`;

    if (mapChart.indicator === 'deaths') return value.toFixed(1);
};
...
const addMapChartLegend = () => {
    ...
    if (mapChart.indicator === 'cost')
        legend
            ...
            .attr('x', legendProperties.x)
            .attr('y', legendProperties.y + legendProperties.height + 20)
            .text('Custo corrigido pelo tamanho da população');
};
...
const addMapChartRanking = () => {
    ...
    ranking
        ...
        .text((d) => {
            if (mapChart.indicator === 'cost')
                return d[mapChart.indicator].toFixed(2);

            if (mapChart.indicator === 'percent')
                return (d[mapChart.indicator] * 100).toFixed(2) + '%';

            if (mapChart.indicator === 'deaths')
                return d[mapChart.indicator].toFixed(1);
        })
        .on('mouseenter', (e, d) => highlightMapAreaAndRanking(d.state))
        .on('mouseleave', (e, d) => unhighlightMapAreaAndRanking(d.state));
